...
void loadData(std::fstream * file_images, std::fstream * file_labels, std::vector<ExampleChar> * data, int inSize, int outSize)
{
	...
	if(magicNum_images==2051 && magicNum_labels==2049){
		...
		for (int i = 0; i < itemCount_images; i++) {
			ExampleChar tmpchar = ExampleChar(inSize, outSize);
			byte label;
...
class MnistLoader
{
	public:
		...
		MnistLoader(const std::string path_train_images, 
					...
					const std::string path_test_labels, int inSize, int outSize)
		{
			...
			if(file_train_images.is_open()&&file_train_labels.is_open()){
				loadData(&file_train_images, &file_train_labels, &train_data, inSize, outSize);			
			...
			if (file_test_labels.is_open()&&file_test_images.is_open()){
				loadData(&file_test_images, &file_test_labels, &test_data, inSize, outSize);			
